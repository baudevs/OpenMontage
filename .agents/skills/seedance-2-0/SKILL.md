...
| **AnyFast** | `ANYFAST_API_KEY` | `anyfast_video` (model: `seedance-2.0`, `seedance-fast`, `seedance-2.0-mini`, `seedance-2.0-ultra`) | ✅ wrapped | Gateway route on one async endpoint. 2.0 reaches 4k, Ultra reaches 2k (resolution required), Fast/Mini are 480p/720p. Billed per generation; set `ANYFAST_VIDEO_PRICE_USD*` for real estimates. |
